    ...
    return (
        <div key={cred.id} className="bg-white/10 p-4 rounded-lg shadow-md border border-white/20 mt-4">
            <div className="flex flex-col md:flex-row items-center justify-between">
                <div>
                    <h3 className="text-xl font-semibold">{cred.name}</h3>
                    <p className="text-gray-400">ID: {cred.id}</p>
                    <p className="text-gray-400 text-sm">Created: {new Date(cred.createdAt).toLocaleDateString()}</p>
                </div>
