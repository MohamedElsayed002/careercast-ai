...
export interface DotPatternProps {
  ...
  /**
   * `fullscreen` — fixed viewport layer (default). `section` — in-flow hero / block.
   */
  layout?: "fullscreen" | "section"
  /** Edge fade overlay. Off for light section heroes. Default: true for fullscreen, false for section. */
  vignette?: boolean
...
export function DotPattern({
  ...
  const vignette = vignetteProp ?? !isSection
  ...
  return (
    ...
    >
      ...
      {children && (
        <div
          className={cn("relative z-10 w-full", !isSection && "h-full")}
        >
          {children}
        </div>
      )}
