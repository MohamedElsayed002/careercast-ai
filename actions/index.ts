...
REMEMBER: The total word count across all dialogue.text fields MUST be at least ${minWords} words. Count carefully!`
    ...
    try {
        ...
        if (totalWords < minWords) {
            Sentry.logger.warn(`Generated ${totalWords} words, but target was ${minWords}-${maxWords} for ${targetMinutes}-minute podcast`)
...
- Do not stop generating until ALL sections are complete`;
    ...
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : "Invalid Error"
        const errorCause = error instanceof Error ? error.cause : "Invalid Error"
